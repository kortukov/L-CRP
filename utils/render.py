...
@torch.no_grad()
def vis_opaque_img_border_v2(data_batch, heatmaps, rf=False, alpha=0.5, vis_th=0.05, crop_th=0.01,
                          kernel_size=13, min_crop_size=30, max_crop_ratio=0.05) -> Image.Image:
    """
    Draws reference images. The function lowers the opacity in regions with relevance lower than max(relevance)*vis_th.
    In addition, the reference image can be cropped where relevance is less than max(relevance)*crop_th by setting 'rf' to True.
    ...
    for i in range(len(data_batch)):
        ...
        # Prevents crashes on edge cases (empty tensors, all zeros, etc.)
        if heatmap_i.dim() < 2 or heatmap_i.numel() == 0:
            continue

        heat_abs_max = heatmap_i.abs().max().item()
        heat_nonzero = (heatmap_i != 0).sum().item()

        if heat_abs_max == 0 or heat_nonzero == 0:
            continue

        # Apply gaussian blur to smooth the heatmap
        filtered_heat = gaussian_blur(heatmap_i.unsqueeze(0), kernel_size=kernel_size)[0]
        filtered_max = filtered_heat.clamp(min=0).max().item()

        # Falls back to absolute values if positive max is too small
        if filtered_max <= 1e-12:
            filtered_heat = filtered_heat.abs()
            filtered_max = filtered_heat.max().item()
            if filtered_max <= 1e-12:
                continue

        # Normalize to [0, 1]
        filtered_heat = filtered_heat / filtered_max
        vis_mask = filtered_heat > vis_th

        vis_ratio = vis_mask.float().mean().item()

        # Prevents mostly black/empty images
        if vis_ratio < 0.001:
            vis_mask = filtered_heat > 0.01
            vis_ratio = vis_mask.float().mean().item()
            if vis_ratio < 0.0001:
                continue
        if rf:
            ...
            if row1 >= row2 or col1 >= col2 or row1 < 0 or col1 < 0:
                continue

            crop_h = row2 - row1
            crop_w = col2 - col1

            # max_crop_ratio controls how much of the image we keep:
            max_h = int(H * max_crop_ratio)
            max_w = int(W * max_crop_ratio)

            if crop_h > max_h or crop_w > max_w:
                # Find center of the relevant region and crop around it
                center_row = (row1 + row2) // 2
                center_col = (col1 + col2) // 2

                half_h = min(crop_h, max_h) // 2
                half_w = min(crop_w, max_w) // 2

                row1 = max(0, center_row - half_h)
                row2 = min(H, center_row + half_h)
                col1 = max(0, center_col - half_w)
                col2 = min(W, center_col + half_w)

                crop_h = row2 - row1
                crop_w = col2 - col1

            # min_crop_size prevents crops that are too small to see:
            if crop_h < min_crop_size:
                center_row = (row1 + row2) // 2
                row1 = max(0, center_row - min_crop_size // 2)
                row2 = min(H, row1 + min_crop_size)
                row1 = max(0, row2 - min_crop_size)

            if crop_w < min_crop_size:
                center_col = (col1 + col2) // 2
                col1 = max(0, center_col - min_crop_size // 2)
                col2 = min(W, col1 + min_crop_size)
                col1 = max(0, col2 - min_crop_size)
